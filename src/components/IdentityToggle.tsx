...
export function IdentityToggle({ identity, onToggle, disabled }: IdentityToggleProps) {
  return (
    <div className="flex items-center justify-center p-2">
      <div className="relative flex w-48 sm:w-72 bg-black border border-border shadow-inner p-1">
        <button
          onClick={onToggle}
          disabled={disabled || identity === 'bruce'}
          className={`flex-1 flex items-center justify-center space-x-2 py-2 transition-all duration-300 font-mono tracking-widest text-[10px] sm:text-xs ${
            identity === 'bruce' 
